...
test('program list dialogs render API data and close cleanly', async ({ page }) => {
  ...
  await noteDialog.getByRole('textbox').fill('更新后的备注')
  await noteDialog.getByRole('button', { name: '保存', exact: true }).click()
  await expect(noteDialog).toBeHidden()

  await page.getByTitle('编辑标签').click()
  const tagsDialog = page.locator('.showcase-tags-dialog')
  await expect(tagsDialog).toBeVisible()
  await tagsDialog.getByPlaceholder('输入自定义标签后点击添加').fill('新标签')
  await tagsDialog.getByRole('button', { name: '添加', exact: true }).click()
