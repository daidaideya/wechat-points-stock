  ...
  <div class="page-stack ql-crons-page">
    <section class="toolbar-card ql-hero">
      <div class="ql-hero-head">
        <div>
          ...
          <p class="section-description">
            查看青龙脚本每日执行时间线，识别间隔过密的任务，并一键重排执行间隔。
          </p>
        </div>
      ...
      <div class="points-hero-meta">
        ...
        <span v-if="excludedCronNames.size" class="points-meta-chip ql-excluded-meta-chip">
          ...
          <el-button link size="small" type="primary" class="ql-clear-excluded-btn" @click="clearAllExcluded">清空</el-button>
        </span>
      </div>
      ...
      <div class="ql-organize-form">
        ...
        <div class="ql-form-actions">
          ...
          <span class="ql-form-hint">
            ...
            共 {{ organizeTargetCrons.length }} 个{{ planForm.codeOnly ? ' code 版' : '启用' }}脚本将参与重排，每日执行 2 次
          </span>
        </div>
      </div>
      <el-alert
      ...
      <template #header>
        <div class="ql-card-header">
          <div>
            ...
            <div class="ql-card-subtitle">按每日最早触发时间排序；标签表示与前一个任务的间隔，标黄小于 {{ planForm.intervalMinutes }} 分钟，标绿大于 {{ planForm.intervalMinutes }} 分钟</div>
          </div>
        ...
        <div v-for="group in filteredGroups" :key="group.hour" class="ql-hour-group">
          ...
          <div class="ql-hour-rows">
            <div
              ...
              :key="cronKey(cron)"
              class="ql-cron-row"
              :class="{ disabled: cron.is_disabled === 1, crowded: isCrowded(cron), sparse: isSparse(cron), excluded: isExcluded(cron) }"
            >
              <div class="ql-cron-main">
                <div class="ql-cron-name-row">
                  <span class="ql-cron-name">{{ cron.name || '(未命名)' }}</span>
                  <el-tag v-if="cron.is_disabled === 1" size="small" type="danger" effect="plain">已禁用</el-tag>
                  <el-tag v-if="isExcluded(cron)" size="small" type="info" effect="plain">已排除</el-tag>
                  <el-tag v-if="isCrowded(cron)" size="small" type="warning" effect="plain">
                    间隔 {{ minuteGap(cron) }} 分钟
                  </el-tag>
                  <el-tag v-else-if="isSparse(cron)" size="small" type="success" effect="plain">
                    间隔 {{ minuteGap(cron) }} 分钟
                  </el-tag>
                </div>
                <div class="ql-cron-command">{{ cron.command }}</div>
              </div>
              <div class="ql-cron-schedule">
                <div class="ql-cron-times">
                  <el-tag
                    v-for="timeText in cron.displayTimes"
                    :key="timeText"
                    size="small"
                    round
                    effect="plain"
                  >{{ timeText }}</el-tag>
                  <el-tag v-if="cron.hasMoreTimes" size="small" round effect="plain" type="info">
                    +{{ cron.moreTimesCount }}
                  </el-tag>
                </div>
                <div class="ql-cron-raw">{{ cron.schedule }}</div>
              </div>
              <div class="ql-cron-actions">
                <el-button size="small" text type="primary" @click="openEditDialog(cron)">修改时间</el-button>
                <el-button size="small" text :type="isExcluded(cron) ? 'warning' : 'info'" @click="toggleExclude(cron)">
                  {{ isExcluded(cron) ? '取消排除' : '排除' }}
                </el-button>
              </div>
            </div>
          </div>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="applying" :disabled="!editPreviewTimes.length" @click="applySingleEdit">保存</el-button>
      </template>
    </el-dialog>
    ...
    >
      <div class="ql-plan-summary">
        ...
        <span v-if="planItems.length">时间窗 {{ planItems[0].newTime.split(' / ')[0] }} ~ {{ planItems[planItems.length - 1].newTime.split(' / ')[0] }}</span>
      </div>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="applying" @click="applyPlan">应用到青龙</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const codeCronsCount = computed(() =>
  scriptCrons.value.filter(isCodeCron).length
)

const otherCronsCount = computed(() =>
  scriptCrons.value.filter((cron) => !isCodeCron(cron)).length
)

const scriptCrons = computed(() =>
  crons.value.filter((cron) => cron.is_system !== 1)
)
...
const disabledCrons = computed(() =>
  scriptCrons.value.filter((cron) => cron.is_disabled === 1)
)
...
const visibleCrons = computed(() => {
  ...
  return scriptCrons.value
    ...
    .filter((cron) => {
      if (!text) return true
      return (
        (cron.name || '').toLowerCase().includes(text) ||
        (cron.command || '').toLowerCase().includes(text)
      )
    })
    .map((cron) => {
...
const editPreviewTimes = computed(() =>
  parseDailyMinutes(editScheduleText.value).map(formatMinute)
)
...
async function applySingleEdit() {
  ...
  const ok = await applyBatch(
    [{ id: editingCron.value.id, schedule: editScheduleText.value.trim() }],
    '执行时间已更新'
  )
  if (ok) {
...
async function applySinglePlanItem(row) {
  ...
  try {
    await ElMessageBox.confirm(
      `将把「${row.name}」的执行时间更新为 ${row.newTime}，确认继续？`,
      '单个应用',
