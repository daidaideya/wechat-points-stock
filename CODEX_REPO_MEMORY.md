...
- 分支：`main`
- 快照提交：`d71108f`（2026-09-19）
- 最新代码提交：`f98ac21`，StockPage 主库存中心已统一使用 `useStockCenterRequest` 管理首屏缓存、同参数在途去重、筛选/强刷取消、卸载取消和旧响应隔离；ProgramsPage 列表/详情/库存读取仍由共享取消控制器保护，API 日期解析、Asia/Shanghai 展示和本地日期判断仍由 `frontend/src/utils/date.js` 统一提供；本批路线图和仓库记忆文档提交为 `d71108f`。
...
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `112 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `53 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `30 passed`，真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
