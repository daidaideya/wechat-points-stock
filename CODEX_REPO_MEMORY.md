...
- 分支：`main`
- 快照提交：`0220666`（路线图已同步；本记忆文档随本批提交推送，2026-09-19）
- 最新代码提交：`3893e86`，将青龙页顶部概览拆为 `QinglongCronOverview.vue`；页面继续保留请求、筛选、黑名单、时间建议、一键整理和弹窗副作用编排。
...
- 前端 Node 内置测试目前共 `36 passed`，Vitest 组件测试共 `83 passed`，其中包含 Programs/Apps 筛选参数、访问会话、页面状态缓存版本/TTL、API 错误解析、AbortError、日期边界、Users 身份展示规则、Stock 中心请求缓存/去重/取消边界、Stock 商品详情抽屉展示/事件边界和 QingLong 概览展示/事件边界测试。
