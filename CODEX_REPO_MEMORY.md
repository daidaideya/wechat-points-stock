...
- 分支：`main`
- 快照提交：`b7fa968`（2026-09-19）
- 最新代码提交：`cecdd44`，新增 `StockManagementDrawers.vue` 统一承载 Stock 页已隐藏/已下架商品抽屉、搜索、加载/空态、恢复显示/恢复上架和分页展示；`StockPage.vue` 收敛为库存中心请求、筛选分页、标准化和商品状态副作用编排，页面约 1263 行降至 1057 行；此前 ProgramMaintenanceDialogs、QingLong 时间线、Stock 筛选工具栏、Points 详情双抽屉、金额精度和共享日期工具仍保持不变；路线图本批提交为 `618d897`，仓库记忆快照提交为 `b7fa968`。
