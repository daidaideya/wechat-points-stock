...
- 分支：`main`
- 快照提交：`410abcf`（路线图已同步；记忆快照随本次提交推送，2026-09-19）
- 最新代码提交：`620ee7c`，清理 `ProgramsPage.vue` 重复卡片 CSS 并以 smoke 断言锁定子组件样式归属；路线图提交为 `46e5249`，页面继续保留请求、筛选、分页和弹窗副作用编排。
...
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `123 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `76 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `30 passed`，使用临时 SQLite、独立 `8001` 后端和 `VITE_API_PROXY_TARGET` 的真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
...
- 前端 Node 内置测试目前共 `36 passed`，Vitest 组件测试共 `80 passed`，其中包含 Programs/Apps 筛选参数、访问会话、页面状态缓存版本/TTL、API 错误解析、AbortError、日期边界、Users 身份展示规则、Stock 中心请求缓存/去重/取消边界和 Stock 商品详情抽屉展示/事件边界测试。
