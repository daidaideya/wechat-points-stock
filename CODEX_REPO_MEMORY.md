...
- 最新代码提交：`96e264c`，新增 `ProgramMaintenanceDialogs.vue` 统一承载 Programs/Apps 备注/标签维护弹窗、本地草稿、快捷/自定义标签和响应式宽度；`ProgramsPage.vue` 收敛为列表筛选、API 请求、标签聚合、保存 pending、详情/库存及其他副作用编排，页面约 2067 行降至 1696 行；Vite `/api`/`/static` 代理默认仍指向 `8000`，可通过 `VITE_API_PROXY_TARGET` 指向隔离后端；此前 QingLong 时间线、Stock 筛选工具栏、Points 详情双抽屉、金额精度和共享日期工具仍保持不变；路线图本批提交为 `a9fe0bd`，仓库记忆快照提交待本次收尾后回填。
...
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `123 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `64 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `30 passed`，使用临时 SQLite、独立 `8001` 后端和 `VITE_API_PROXY_TARGET` 的真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
...
- 2026-09-19 将 Programs/Apps 备注和标签维护弹窗提取为 `ProgramMaintenanceDialogs.vue`，父页继续保留当前程序、列表标签聚合、PUT 请求和 pending；新增组件测试及三浏览器保存 smoke，Vite 代理增加可选隔离后端目标；代码提交为 `96e264c`，路线图提交为 `a9fe0bd`，记忆快照提交待本次收尾后回填。
