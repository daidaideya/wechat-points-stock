...
- 分支：`main`
- 快照提交：`bdd8adb`（2026-09-19）
- 最新代码提交：`e11818d`，新增 `ProgramCardActions.vue` 和对应组件测试，将 Programs/Apps 卡片操作区及响应式样式从 `ProgramCard.vue` 拆出；父组件由 784 行收敛为 510 行，继续通过白名单事件回传，页面仍保留所有 API/状态副作用编排；路线图本批提交为 `2315161`，仓库记忆快照提交为 `bdd8adb`。
