...
- 分支：`main`
- 快照提交：`65c9ece`（2026-09-19）
- 最新修复：QingLong 时间线父页补齐 `:cron="cron"` 数据绑定，任务名称、命令和时间不再全部显示为空；代码提交为 `65c9ece`，三浏览器 E2E smoke 当前为 `21 passed`。此前 Stock 卡片布局和 Points 导航问题已保留在历史提交中。
...
- 本次推进：OPT-014 追加 QingLong 任务行数据绑定修复（第二十八批），OPT-017 追加 QingLong 任务名称页面回归（第三十二批）；同时保留上一批 Stock 商品卡的独立宽度容器和上下结构修复。
- 前端构建：已执行 `npm run build` 通过；构建会生成/刷新 `frontend/dist`
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `112 passed`；前端 Node 测试 `npm test` 为 `30 passed`，Vitest 组件测试为 `47 passed`，API 类型漂移检查、Playwright mock Chromium + Firefox + WebKit smoke 共 `21 passed`（含 dialog ARIA/焦点回收、ProgramFilterBar 筛选状态、ProgramMetricStrip 快照告警、ProgramCard 展示与事件、ProgramDetailDialog 加载/关闭/库存入口、ProgramStockDialog 库存摘要/变动展开/加载态、UserPointsDialog 积分/现金格式/空态/加载态、StockProductCard 图片/价格/库存/隐藏事件、Stock 商品卡窄桌面布局和指标无重叠、QingLong 任务名称/命令/时间线回归、UserMobileCard 身份/回退/计数/操作事件、UserEditDialog 编辑/新增/字段/保存/关闭事件、UserDesktopTable 多行/回退/计数/操作事件、QinglongCronRow 状态/时间/操作事件、Qinglong 整理预览、Settings 导航语义和共享卡片投影、Settings 四个业务区段字段/状态/操作事件、DashboardMetricCard 展示/激活事件、PointsAccountCard 状态/指标/Top 小程序/入口事件断言、Points 双抽屉和移动导航回归），真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、`npm run format:check`、`npm run check:resources` 和 `npm run build` 通过；`py -3.11 scripts/export_openapi.py --check` 通过；npm audit、pip-audit 和许可证检查均通过（新增依赖后检查 335 个 npm 包）
