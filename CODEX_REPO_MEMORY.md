...
- 分支：`main`
- 快照提交：待本批文档提交后更新（2026-09-19）
- 最新代码提交：`b3b1604`，StockPage 主库存中心已统一使用 `useStockCenterRequest` 管理首屏缓存、同参数在途去重、筛选/强刷取消、卸载取消和旧响应隔离；现金写入与 `cash_max` 查询已统一使用 `app/money.py` 的 Decimal 两位小数半入规则；ProgramsPage 列表/详情/库存读取仍由共享取消控制器保护，API 日期解析、Asia/Shanghai 展示和本地日期判断仍由 `frontend/src/utils/date.js` 统一提供；本批路线图和 API 文档提交待推送。
...
- 2026-09-19 新增 `app/money.py`，以 Decimal 和 `ROUND_HALF_UP` 将现金金额统一归一化到两位小数；接入积分现金上报、库存上报、手工库存 CRUD 与 `cash_max` 筛选，旧 Float/REAL 列和历史值保持兼容。新增 `tests/test_money_precision.py` 及库存筛选回归，后端全量 `123 passed`；代码提交为 `b3b1604`，路线图、API 文档和记忆快照提交待推送。
