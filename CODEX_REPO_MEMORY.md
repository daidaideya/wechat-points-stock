...
- 分支：`main`
- 快照提交：`0ea0602`（路线图已同步；记忆快照随本次提交推送，2026-09-19）
- 最新代码提交：`ec22da2`，修复 `StockFilterToolbar.vue` 的样式边界，并将 QingLong 排除语义改为按任务 ID 持久化的浏览器端黑名单；路线图提交为 `0ea0602`，父页继续保留筛选、时间建议和批量整理副作用编排。
...
- 前端 Node 内置测试目前共 `36 passed`，其中包含 Programs/Apps 筛选参数、访问会话、页面状态缓存版本/TTL、API 错误解析、AbortError、日期边界、Users 身份展示规则和 Stock 中心请求缓存/去重/取消边界测试。
