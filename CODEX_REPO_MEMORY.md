...
- 分支：`main`
- 快照提交：`3b817b2`（2026-09-19）
- 最新代码提交：`57d7a29`，新增 `QinglongCronTimeline.vue` 统一承载青龙当前时间线卡片、按小时分组、显示禁用开关、loading/empty 和 `QinglongCronRow` 组合；`QinglongCronsPage.vue` 收敛为任务请求、筛选、排除、拥挤度、最新脚本 ID/占用分钟/小时溢出时间建议、编辑和批量写入编排；此前 Stock 主库存请求/筛选工具栏、Points 详情双抽屉、金额精度和共享日期工具仍保持不变；路线图本批提交为 `e9137a3`，仓库记忆快照提交为 `3b817b2`。
