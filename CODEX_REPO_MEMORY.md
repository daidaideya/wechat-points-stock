...
- 分支：`main`
- 快照提交：`e8300d9`（2026-09-19）
- 最新代码提交：`0619633`，新增 `ProgramStockTable.vue` 和对应组件测试，将 Programs/Apps 库存弹窗的商品表格展示与响应式样式从 `ProgramStockDialog.vue` 拆出；父弹窗由 678 行收敛为 523 行，继续通过函数 props 接收展示计算，页面仍保留库存 API/排序/兑换业务编排；同时修复 `App.vue` 移动导航在动态 resize 后读取旧响应式宽度的竞态。路线图本批提交为 `fdd940e`，仓库记忆快照提交为 `e8300d9`。
