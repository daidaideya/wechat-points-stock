...
- 分支：`main`
- 快照提交：`6fccf2c`（2026-09-19）
- 最新代码提交：`6fccf2c`，共享 `useViewport` 已有初始宽度、跨阈值 resize 和卸载清理测试；App 根布局移动导航仍覆盖桌面到 390px 的动态断点切换，真实后端 smoke 稳定性、Programs/Apps 共享视口、Stock/QingLong 读取保护和 Users smoke 修复继续保留。
