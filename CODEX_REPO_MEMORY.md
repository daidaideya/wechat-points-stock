...
- 分支：`main`
- 快照提交：`3e89bc0`（2026-09-19）
- 最新代码提交：`20e1269`，StockPage 主库存中心已统一使用 `useStockCenterRequest` 管理首屏缓存、同参数在途去重、筛选/强刷取消、卸载取消和旧响应隔离；库存页搜索、标签、价格/现金上限、库存状态和结果摘要已拆到 `StockFilterToolbar.vue`；现金写入与 `cash_max` 查询已统一使用 `app/money.py` 的 Decimal 两位小数半入规则；ProgramsPage 列表/详情/库存读取仍由共享取消控制器保护，PointsPage 的详情与未注册抽屉已拆到 `PointsAccountDetailsDrawer.vue`；API 日期解析、Asia/Shanghai 展示和本地日期判断仍由 `frontend/src/utils/date.js` 统一提供；本批路线图提交为 `dde072d`，仓库记忆快照待本批提交后更新。
...
- 2026-09-19 将库存页搜索、标签、价格/现金上限、库存状态和结果摘要提取为 `StockFilterToolbar.vue`，父页保留 `useStockFilters`、库存中心请求/缓存、分页和商品操作；新增组件测试覆盖受控输入及筛选/抽屉/刷新事件，三浏览器 Playwright `30 passed`、Vitest `58 passed`；代码提交为 `20e1269`，路线图提交为 `dde072d`，记忆快照待本批提交后更新。
