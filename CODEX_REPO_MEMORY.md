...
- 最新代码提交：`57d7a29`，新增 `QinglongCronTimeline.vue` 统一承载青龙当前时间线卡片、按小时分组、显示禁用开关、loading/empty 和 `QinglongCronRow` 组合；`QinglongCronsPage.vue` 收敛为任务请求、筛选、排除、拥挤度、最新脚本 ID/占用分钟/小时溢出时间建议、编辑和批量写入编排；此前 Stock 主库存请求/筛选工具栏、Points 详情双抽屉、金额精度和共享日期工具仍保持不变；路线图本批提交为 `e9137a3`，仓库记忆快照提交待本次收尾后回填。
...
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `123 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `61 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `30 passed`，真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
...
- 2026-09-19 将 QingLong 当前时间线卡片、禁用开关、按小时分组、loading/empty 和 `QinglongCronRow` 组合提取为 `QinglongCronTimeline.vue`，父页继续保留任务请求、筛选、排除、时间建议和批量写入；新增时间线组件测试，三浏览器 Playwright `30 passed`、Vitest `61 passed`；代码提交为 `57d7a29`，路线图提交为 `e9137a3`，记忆快照待本次收尾提交后回填。
