...
- 本次推进：OPT-014 第三十四批将 StockPage 主库存中心的缓存、同参数在途去重和取消生命周期收敛到 `useStockCenterRequest`；OPT-015 第二十批补齐筛选/强刷/卸载取消与取消错误静默；OPT-017 第四十五批新增控制器单测，Node 测试提升到 `36 passed`。
- 本次推进：OPT-016 第三批新增 `app/money.py`，统一现金金额两位小数 `ROUND_HALF_UP` 归一化并接入积分/库存写入与 `cash_max` 查询；OPT-018 第六批同步 API 与库存上报文档，后端全量测试提升到 `123 passed`。
- 本次推进：OPT-014 第三十五批新增 `PointsAccountDetailsDrawer.vue`，将积分详情与未注册小程序两个抽屉从 `PointsPage.vue` 拆出；OPT-017 第四十六批新增双抽屉组件测试，Vitest 提升到 `55 passed`，并通过三浏览器 smoke 保持无 console warning。
- 本次推进：OPT-014 第三十六批新增 `StockFilterToolbar.vue`，将库存页搜索、标签、价格/现金上限、库存状态和结果摘要从 `StockPage.vue` 拆出；OPT-017 第四十七批新增筛选工具栏组件测试，Vitest 提升到 `58 passed`，三浏览器 smoke 保持 `30 passed`。
- 本次推进：OPT-014 第三十七批新增 `QinglongCronTimeline.vue`，将青龙当前时间线卡片、分组、禁用开关和行组合从 `QinglongCronsPage.vue` 拆出，父页继续保留任务请求、筛选、时间建议和批量写入；OPT-017 第四十八批新增时间线组件测试，Vitest 提升到 `61 passed`，三浏览器 smoke 保持 `30 passed`。
- 本次推进：OPT-014 第三十八批新增 `ProgramMaintenanceDialogs.vue`，将 Programs/Apps 的备注和标签维护弹窗、本地草稿及响应式展示从 `ProgramsPage.vue` 拆出，父页继续保留 PUT 请求、列表标签聚合和 pending；OPT-017 第四十九批新增维护弹窗组件测试及保存 smoke，Vitest 提升到 `64 passed`，三浏览器 smoke 保持 `30 passed`，Vite 代理支持隔离后端目标。
- 本次推进：OPT-014 第三十九批新增 `StockManagementDrawers.vue`，将 Stock 页已隐藏/已下架商品抽屉、搜索、加载/空态、恢复显示/恢复上架和分页展示从 `StockPage.vue` 拆出，父页继续保留请求取消、分页聚合、缓存失效和商品状态副作用；OPT-017 第五十批新增管理抽屉组件测试，Vitest 提升到 `67 passed`，三浏览器 smoke 保持 `30 passed`，隔离真实后端 Chromium smoke 保持 `1 passed`。
- 本次推进：OPT-014 第四十批新增 `QinglongCronEditDialog.vue`，将青龙任务执行时间编辑弹窗、cron 输入预览、非法表达式提示和响应式宽度从 `QinglongCronsPage.vue` 拆出，父页继续保留 cron 解析、保存 API、loading 和成功刷新；OPT-017 第五十一批新增编辑弹窗组件测试，Vitest 提升到 `70 passed`，三浏览器 smoke 保持 `30 passed`。
...
- 本次推进：OPT-014 第五十六批新增 `StockProductDetailDrawer.vue`，将商品详情抽屉的图片回退、身份、库存/价格/余额指标、兑换状态和响应式样式从 `StockPage.vue` 拆出；商品卡通过 `open-detail` 回传，父页继续保留选中商品、抽屉状态、格式化函数和库存副作用编排。OPT-017 第五十六批新增详情抽屉与商品卡事件测试，Vitest 提升到 `80 passed`，三浏览器 smoke 保持 `30 passed`，真实后端 Chromium smoke 保持 `1 passed`。
- 本次推进：OPT-014 第五十七批清理 `ProgramsPage.vue` 在卡片、操作区和指标组件拆分后遗留的重复 scoped CSS，仅保留页面背景、网格、空态跨列和无限滚动状态栏；页面由 1696 行收敛为 871 行，业务请求、筛选、分页和弹窗编排不变。OPT-017 第五十七批新增 Programs 卡片样式归属 smoke 断言，Vitest 保持 `80 passed`，三浏览器 smoke 保持 `30 passed`，真实后端 Chromium smoke 保持 `1 passed`。
- 本次推进：OPT-014 第五十八批新增 `QinglongCronOverview.vue`，将青龙页顶部指标、新脚本时间建议展示、搜索和脚本类型筛选从 `QinglongCronsPage.vue` 拆出；父页继续保留任务 API、黑名单、最新脚本 ID/占用分钟/小时溢出建议、一键整理、时间线和弹窗副作用。OPT-017 第五十八批新增概览组件测试，Vitest 提升到 `83 passed`，三浏览器 smoke 保持 `30 passed`，真实后端 Chromium smoke 保持 `1 passed`。
- 本次推进：OPT-014 第五十九批新增 `DashboardActivityPanels.vue`，将 Dashboard 最近更新、今日未报列表和完整未报详情弹窗从 `DashboardPage.vue` 拆出；父页继续保留指标计算、Dashboard/未报 API、请求取消、路由和弹窗状态编排。OPT-017 第五十九批新增活动区组件测试，Vitest 提升到 `86 passed`，三浏览器 smoke 保持 `30 passed`，真实后端 Chromium smoke 保持 `1 passed`。
- 本次推进：OPT-014 第六十批新增 `UsersListSection.vue`，将 Users 用户列表标题/排序提示、加载/空态、移动卡片和桌面表格从 `UsersPage.vue` 拆出；通过显式桥接保留父页 Sortable 的 DOM 访问，父页继续保留用户请求、表单/弹窗和排序/删除副作用。OPT-017 第六十批新增列表区组件测试，Vitest 提升到 `89 passed`，三浏览器 smoke 保持 `30 passed`，真实后端 Chromium smoke 保持 `1 passed`。
- 本次修复：OPT-014 第六十一批修复 Programs/Apps 窄三列卡片的文本裁切，`ProgramCard.vue` 的标题、`program_id` 和标签改为完整换行展示；OPT-017 第六十一批新增超长中文名称 Playwright 回归，Vitest 保持 `89 passed`，三浏览器 smoke 保持 `30 passed`，真实后端 Chromium smoke 保持 `1 passed`。
- 本次修复：OPT-014 第六十二批继续修复 Programs/Apps 窄三列卡片的布局挤压；卡片宽度不超过 220px 时，`ProgramCard.vue` 用 container query 将序号和青龙状态独立到顶行，标题占满内容宽度，避免长中文名称一字一行。OPT-017 第六十二批将窄布局回归固定为 960px 视口，移除易受字体/浏览器字高舍入影响的 `scrollHeight` 断言；Vitest 保持 `89 passed`，三浏览器 smoke 提升到 `33 passed`，真实后端 Chromium smoke 保持 `1 passed`。
...
- 本次推进：OPT-018 追加业务 API 参考文档（第五批），README 已增加文档入口；本批未改动运行时代码，继续以 OpenAPI 生成基线和现有前后端门禁验证文档没有造成契约漂移。
- 前端构建：已执行 `npm run build` 通过；构建会生成/刷新 `frontend/dist`
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `123 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `89 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `33 passed`，使用临时 SQLite、独立 `8001` 后端和 `VITE_API_PROXY_TARGET` 的真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
...
| `app/money.py` | 现金金额 Decimal 两位小数 `ROUND_HALF_UP` 归一化，兼容数字字符串和旧 Float/REAL 存储 |
...
| `frontend/src/composables/useStockCenterRequest.js` | Stock 主库存中心请求的首屏缓存、同参数去重、取消和卸载生命周期 |
| `frontend/src/composables/useStockCenterRequest.test.js` | 库存中心请求控制器的缓存、去重、取消和旧请求晚到边界测试 |
...
| `frontend/src/components/ProgramCardActions.vue` | Programs/Apps 卡片日期脚注、移动/桌面操作和归档/删除菜单；只接收状态并通过白名单事件回传，不直接访问 API |
| `frontend/src/components/ProgramMaintenanceDialogs.vue` | Programs/Apps 备注/标签维护弹窗、响应式宽度、快捷/自定义标签和本地草稿；通过保存与可见性事件回到页面编排 |
...
| `frontend/src/components/ProgramStockDialog.vue` | Programs/Apps 库存摘要、变化明细和弹窗交互；通过函数 props 接收库存展示计算，并组合 `ProgramStockTable` |
| `frontend/src/components/ProgramStockTable.vue` | Programs/Apps 在架商品表格、图片预览、兑换状态、积分差额和响应式表格样式；只接收排序数据与展示函数，不访问 API |
| `frontend/src/components/ProgramStockTable.component.spec.js` | 在架商品表格数据、行 class 回调和空列表边界组件测试 |
| `frontend/src/components/StockProductCard.vue` | Stock 页主库存列表的商品卡片展示；通过 `open-detail` 回传详情入口、通过 `hide` 回到页面执行隐藏操作 |
| `frontend/src/components/StockProductDetailDrawer.vue` | Stock 页商品详情抽屉、图片回退、身份/状态/库存/价格指标和响应式展示；通过 `v-model` 回传可见性 |
| `frontend/src/components/StockProductDetailDrawer.component.spec.js` | 商品详情抽屉有图/无图、状态标签、指标格式化、关闭事件和隐藏态组件测试 |
| `frontend/src/components/QinglongCronOverview.vue` | QingLong 顶部指标、新脚本时间建议展示、搜索、拥挤/黑名单摘要和脚本类型筛选；通过刷新、复制、清空黑名单和受控筛选事件回到父页 |
| `frontend/src/components/QinglongCronOverview.component.spec.js` | QingLong 概览指标/建议/筛选展示、刷新/复制/黑名单/筛选事件和无建议禁用边界组件测试 |
| `frontend/src/components/QinglongCronTimeline.vue` | QingLong 当前时间线卡片、启用/禁用开关、按小时分组、loading/empty 和行组合；父页通过 props 注入计算函数并接收交互事件 |
...
| `frontend/src/components/UsersSummaryCards.vue` | Users 页用户数、活跃小程序/APP、手机号统计卡展示和响应式布局；只接收父页计算后的数值 |
| `frontend/src/components/UsersSummaryCards.component.spec.js` | Users 四项统计卡展示和数值更新组件测试 |
| `frontend/src/components/UsersListSection.vue` | Users 用户列表标题、排序提示、加载/空态、移动卡片和桌面表格展示；通过白名单事件和 Sortable DOM 桥接回到页面编排 |
| `frontend/src/components/UsersListSection.component.spec.js` | Users 列表展示、排序提示、移动/桌面操作事件、加载态和空态组件测试 |
| `frontend/src/components/DashboardActivityPanels.vue` | Dashboard 最近更新、今日未报列表和完整未报详情弹窗展示；通过刷新和 `v-model:unreported-dialog-visible` 回到页面编排 |
| `frontend/src/components/DashboardActivityPanels.component.spec.js` | Dashboard 活动列表、日期格式化、刷新/可见性事件、加载态和空态组件测试 |
...
- `ProgramCard.vue` 负责 Programs/Apps 程序卡片的名称/ID、青龙状态、标签、指标和备注展示及自身卡片样式；`ProgramCardActions.vue` 负责日期脚注、移动/桌面操作、归档/删除菜单及自身操作区样式，`ProgramMetricStrip.vue` 负责指标展示和样式；`ProgramsPage.vue` 只保留页面网格/背景/空态布局以及复制、标签、库存、详情、备注、收藏、归档和删除等业务副作用。
- `ProgramCard.vue` 的标题、`program_id` 和标签在窄三列布局下使用可换行的完整展示策略，不再依赖单行 ellipsis；卡片宽度不超过 220px 时通过 container query 将序号/青龙状态独立到顶行，标题占满内容宽度；`ProgramsPage.vue` 的 `titleSizeClass` 只负责按长度缩小字号，长名称仍可换行。
- `DashboardActivityPanels.vue` 负责 Dashboard 最近更新、今日未报摘要和完整未报详情弹窗的展示、加载/空态与响应式布局；`DashboardPage.vue` 继续保留指标计算、Dashboard/未报 API、请求取消、路由跳转和弹窗数据状态，组件通过刷新和 `v-model:unreported-dialog-visible` 回传交互。
- `UsersListSection.vue` 负责 Users 用户列表标题、排序提示、加载/空态、移动卡片和桌面表格组合；`UsersPage.vue` 继续保留用户 API、表单/积分弹窗、删除和 Sortable 写入，组件通过用户操作事件以及 `getMobileListElement()`/`getDesktopTableInstance()` 桥接回传展示 DOM。
- `ProgramMaintenanceDialogs.vue` 负责 Programs/Apps 备注和标签维护弹窗、本地编辑草稿、快捷/自定义标签、响应式宽度和保存/关闭事件；`ProgramsPage.vue` 继续持有当前程序、备注/标签 PUT 请求、列表标签聚合、pending 和成功后的状态更新，组件不直接访问 API。
...
- `StockProductCard.vue` 负责 Stock 页标准化商品对象的图片、状态、价格、库存、最高积分和详情/隐藏入口展示；`StockProductDetailDrawer.vue` 负责商品详情抽屉及自身响应式样式；`StockPage.vue` 保留请求、筛选、分页、商品标准化、选中商品和隐藏 API，卡片通过 `open-detail`/`hide` 事件回传。
...
- `QinglongCronOverview.vue` 负责 QingLong 顶部指标、新脚本时间建议复制入口、关键词搜索、拥挤/黑名单摘要和脚本类型筛选；`QinglongCronsPage.vue` 保留数量与建议计算、任务 API、黑名单持久化、一键整理表单、时间线、编辑/批量写入副作用，概览组件通过刷新、复制、清空黑名单和受控筛选事件回传。
...
- `UsersSummaryCards.vue` 负责 Users 页四张统计卡和响应式网格展示；`UsersPage.vue` 继续计算用户数、活跃小程序/APP 数和留存手机号数，并保留用户列表请求、Sortable、弹窗和写操作副作用。
...
- 前端 Node 内置测试目前共 `36 passed`，Vitest 组件测试共 `89 passed`，其中包含 Programs/Apps 筛选参数、访问会话、页面状态缓存版本/TTL、API 错误解析、AbortError、日期边界、Users 身份展示规则、Stock 中心请求缓存/去重/取消边界、Stock 商品详情抽屉展示/事件边界、QingLong 概览展示/事件边界、Dashboard 活动区展示/事件边界和 Users 列表展示/事件边界测试。
...
- 2026-09-19 新增 `frontend/src/composables/useStockCenterRequest.js`，将 StockPage 主库存中心的首屏缓存、同参数去重、筛选/强刷取消和卸载取消统一到一个传输边界；页面继续负责筛选、分页、标准化和商品操作。新增控制器单测覆盖缓存命中、取消、后续请求和旧请求晚到，代码提交为 `f98ac21`；Node `36 passed`、Vitest `53 passed`、三浏览器 mock `30 passed`、真实后端 Chromium `1 passed`。
- 2026-09-19 新增 `app/money.py`，以 Decimal 和 `ROUND_HALF_UP` 将现金金额统一归一化到两位小数；接入积分现金上报、库存上报、手工库存 CRUD 与 `cash_max` 筛选，旧 Float/REAL 列和历史值保持兼容。新增 `tests/test_money_precision.py` 及库存筛选回归，后端全量 `123 passed`；代码提交为 `b3b1604`，路线图、API 文档和记忆快照提交为 `cfe4e44`。
- 2026-09-19 将积分总览的账号详情与未注册小程序双抽屉提取为 `PointsAccountDetailsDrawer.vue`，父页保留请求、账号标准化、列表筛选和可见性编排；新增组件测试覆盖摘要、排序、关键词筛选、未注册列表和关闭事件。补齐拆分组件的 `Search` 显式图标导入，三浏览器 Playwright `30 passed`、Vitest `55 passed`；代码提交为 `77fc19c`，路线图提交为 `d7d5684`，记忆快照提交为 `3e89bc0`。
- 2026-09-19 将库存页搜索、标签、价格/现金上限、库存状态和结果摘要提取为 `StockFilterToolbar.vue`，父页保留 `useStockFilters`、库存中心请求/缓存、分页和商品操作；新增组件测试覆盖受控输入及筛选/抽屉/刷新事件，三浏览器 Playwright `30 passed`、Vitest `58 passed`；代码提交为 `20e1269`，路线图提交为 `dde072d`，记忆快照提交为 `40c95a1`。
- 2026-09-19 将 QingLong 当前时间线卡片、禁用开关、按小时分组、loading/empty 和 `QinglongCronRow` 组合提取为 `QinglongCronTimeline.vue`，父页继续保留任务请求、筛选、排除、时间建议和批量写入；新增时间线组件测试，三浏览器 Playwright `30 passed`、Vitest `61 passed`；代码提交为 `57d7a29`，路线图提交为 `e9137a3`，记忆快照提交为 `3b817b2`。
- 2026-09-19 将 Programs/Apps 备注和标签维护弹窗提取为 `ProgramMaintenanceDialogs.vue`，父页继续保留当前程序、列表标签聚合、PUT 请求和 pending；新增组件测试及三浏览器保存 smoke，Vite 代理增加可选隔离后端目标；代码提交为 `96e264c`，路线图提交为 `a9fe0bd`，记忆快照提交为 `b348974`。
- 2026-09-19 将 Stock 页已隐藏/已下架商品抽屉提取为 `StockManagementDrawers.vue`，父页继续保留请求取消、服务端分页聚合、缓存失效和恢复/重新上架副作用；新增管理抽屉组件测试，Vitest `67 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`；代码提交为 `cecdd44`，路线图提交为 `618d897`，记忆快照提交为 `b7fa968`。
- 2026-09-19 将 QingLong 任务执行时间编辑弹窗提取为 `QinglongCronEditDialog.vue`，父页继续保留 cron 解析预览、保存 API、loading 和成功刷新；新增编辑弹窗组件测试，Vitest `70 passed`、三浏览器 Playwright `30 passed`；代码提交为 `fca6e63`，路线图提交为 `dac850f`，记忆快照提交为 `1347e09`。
...
- 2026-09-19 修复 StockFilterToolbar 组件拆分后的 scoped 样式穿透问题，将筛选区布局样式归回组件并新增 smoke 断言；青龙“排除”改为按任务 ID（兼容旧名称）的浏览器端持久黑名单，更新行标签、按钮语义和刷新回归，Vitest `76 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`；代码提交为 `ec22da2`，路线图提交为 `0ea0602`。
- 2026-09-19 将 Stock 商品详情抽屉提取为 `StockProductDetailDrawer.vue`，`StockPage.vue` 由 1057 行收敛为 936 行；商品卡新增 `open-detail` 事件和详情入口，新增抽屉组件测试及三浏览器详情打开/关闭 smoke，Vitest `80 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`；代码提交为 `f9cd518`，路线图提交为 `410abcf`，本记忆快照随本批提交。
- 2026-09-19 清理 `ProgramsPage.vue` 在卡片/操作区/指标拆分后遗留的 827 行重复 scoped CSS，页面由 1696 行收敛为 871 行；新增 Programs 卡片样式归属 smoke，完整门禁保持 Node `36 passed`、Vitest `80 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`、后端 `123 passed` 和生产构建通过；代码提交为 `620ee7c`，路线图提交为 `46e5249`，本记忆快照随本批提交。
- 2026-09-19 将 QingLong 顶部指标、新脚本时间建议、搜索和脚本类型筛选提取为 `QinglongCronOverview.vue`，`QinglongCronsPage.vue` 由 1048 行收敛为 700 行；新增概览组件测试，完整门禁保持 Node `36 passed`、Vitest `83 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`、后端 `123 passed` 和生产构建通过；代码提交为 `3893e86`，路线图提交为 `0220666`，本记忆快照随本批提交。
- 2026-09-19 将 Dashboard 最近更新、今日未报列表和详情弹窗提取为 `DashboardActivityPanels.vue`，`DashboardPage.vue` 由 274 行收敛为 165 行；新增活动区组件测试，完整门禁保持 Node `36 passed`、Vitest `86 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`、后端 `123 passed` 和生产构建通过；代码提交为 `310caeb`，路线图提交为 `14a0a4a`，本记忆快照随本批提交。
- 2026-09-19 将 Users 用户列表展示和 Sortable DOM 桥接提取为 `UsersListSection.vue`，`UsersPage.vue` 由 569 行收敛为 490 行；新增列表区组件测试，完整门禁保持 Node `36 passed`、Vitest `89 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`、后端 `123 passed` 和生产构建通过；代码提交为 `b93095d`，路线图提交为 `367c68b`，本记忆快照随本批提交。
- 2026-09-19 修复 Programs/Apps 窄三列卡片标题、`program_id` 和标签被单行省略裁切的问题，改为可换行完整展示；新增超长中文名称 Playwright 回归，完整门禁保持 Node `36 passed`、Vitest `89 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`、后端 `123 passed` 和生产构建通过；代码提交为 `35eb536`，路线图提交为 `1a72f10`，本记忆快照随本批提交。
- 2026-09-19 继续修复 Programs/Apps 窄三列卡片的布局挤压：宽度不超过 220px 时用 container query 将序号/青龙状态独立到顶行，标题占满内容宽度；新增 960px 窄布局回归并移除不稳定的字高断言，完整门禁为 Node `36 passed`、Vitest `89 passed`、三浏览器 Playwright `33 passed`、真实后端 Chromium `1 passed`、后端 `123 passed` 和生产构建通过；代码提交为 `e081473`，路线图提交为 `6edf907`，本记忆快照随本批提交。
