...
- 分支：`main`
- 快照提交：待本次记忆文件提交后回填
- 最新代码提交：`0619633`，新增 `ProgramStockTable.vue` 和对应组件测试，将 Programs/Apps 库存弹窗的商品表格展示与响应式样式从 `ProgramStockDialog.vue` 拆出；父弹窗由 678 行收敛为 523 行，继续通过函数 props 接收展示计算，页面仍保留库存 API/排序/兑换业务编排；同时修复 `App.vue` 移动导航在动态 resize 后读取旧响应式宽度的竞态。路线图本批提交为 `fdd940e`，仓库记忆快照提交待回填。
- 最新修复：移动导航 1120px→390px 动态切换回归已修复，三浏览器 E2E smoke 当前为 `30 passed`；Users 窄内容区、QingLong 任务数据绑定、Stock 卡片布局和 Points 导航问题已保留在历史提交中。
...
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `123 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `74 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `30 passed`，使用临时 SQLite、独立 `8001` 后端和 `VITE_API_PROXY_TARGET` 的真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
...
- 2026-09-19 将 Programs/Apps 库存弹窗在架商品表格提取为 `ProgramStockTable.vue`，`ProgramStockDialog.vue` 由 678 行收敛为 523 行；新增表格组件测试，修复移动导航动态 resize 竞态，Vitest `74 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`；代码提交为 `0619633`，路线图提交为 `fdd940e`，记忆快照提交待回填。
