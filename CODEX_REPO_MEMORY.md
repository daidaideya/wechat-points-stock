...
- 最新代码提交：`f98ac21`，StockPage 主库存中心已统一使用 `useStockCenterRequest` 管理首屏缓存、同参数在途去重、筛选/强刷取消、卸载取消和旧响应隔离；ProgramsPage 列表/详情/库存读取仍由共享取消控制器保护，API 日期解析、Asia/Shanghai 展示和本地日期判断仍由 `frontend/src/utils/date.js` 统一提供；本批文档提交待推送。
