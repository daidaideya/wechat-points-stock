...
- 分支：`main`
- 快照提交：`d7d5684`（2026-09-19）
- 最新代码提交：`77fc19c`，StockPage 主库存中心已统一使用 `useStockCenterRequest` 管理首屏缓存、同参数在途去重、筛选/强刷取消、卸载取消和旧响应隔离；现金写入与 `cash_max` 查询已统一使用 `app/money.py` 的 Decimal 两位小数半入规则；ProgramsPage 列表/详情/库存读取仍由共享取消控制器保护，PointsPage 的详情与未注册抽屉已拆到 `PointsAccountDetailsDrawer.vue`；API 日期解析、Asia/Shanghai 展示和本地日期判断仍由 `frontend/src/utils/date.js` 统一提供；本批路线图提交为 `d7d5684`，仓库记忆快照待本批提交后更新。
...
- 2026-09-19 将积分总览的账号详情与未注册小程序双抽屉提取为 `PointsAccountDetailsDrawer.vue`，父页保留请求、账号标准化、列表筛选和可见性编排；新增组件测试覆盖摘要、排序、关键词筛选、未注册列表和关闭事件。补齐拆分组件的 `Search` 显式图标导入，三浏览器 Playwright `30 passed`、Vitest `55 passed`；代码提交为 `77fc19c`，路线图提交为 `d7d5684`，记忆快照提交待本批提交后更新。
