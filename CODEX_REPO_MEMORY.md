...
- 分支：`main`
- 快照提交：`fb04c97`（2026-09-19）
- 最新修复：Stock 商品卡在窄桌面双列网格中改为按卡片宽度自适应，避免图片、商品信息和价格指标互相覆盖；代码提交为 `fb04c97`，三浏览器 E2E smoke 当前为 `18 passed`。此前 Points 账号卡漏传 `item` 和桌面/移动导航跳转问题已保留在历史提交中。
