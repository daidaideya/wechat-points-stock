...
- 最新代码提交：`cecdd44`，新增 `StockManagementDrawers.vue` 统一承载 Stock 页已隐藏/已下架商品抽屉、搜索、加载/空态、恢复显示/恢复上架和分页展示；`StockPage.vue` 收敛为库存中心请求、筛选分页、标准化和商品状态副作用编排，页面约 1263 行降至 1057 行；此前 ProgramMaintenanceDialogs、QingLong 时间线、Stock 筛选工具栏、Points 详情双抽屉、金额精度和共享日期工具仍保持不变；路线图本批提交为 `618d897`，仓库记忆快照提交待本次收尾后回填。
...
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `123 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `67 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `30 passed`，使用临时 SQLite、独立 `8001` 后端和 `VITE_API_PROXY_TARGET` 的真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
...
- 2026-09-19 将 Stock 页已隐藏/已下架商品抽屉提取为 `StockManagementDrawers.vue`，父页继续保留请求取消、服务端分页聚合、缓存失效和恢复/重新上架副作用；新增管理抽屉组件测试，Vitest `67 passed`、三浏览器 Playwright `30 passed`、真实后端 Chromium `1 passed`；代码提交为 `cecdd44`，路线图提交为 `618d897`，记忆快照提交待本次收尾后回填。
