...
- 分支：`main`
- 快照提交：`367c68b`（路线图已同步；本记忆文档随本批提交推送，2026-09-19）
- 最新代码提交：`b93095d`，将 Users 用户列表展示和 Sortable DOM 桥接拆为 `UsersListSection.vue`；页面继续保留请求、表单/弹窗和排序/删除副作用编排。
- 最新修复：库存筛选栏已恢复 flex、圆角和内边距样式；青龙黑名单刷新后仍保持；Stock 商品详情抽屉已可从商品卡打开并通过 Escape 关闭；Programs 卡片在父页样式清理后仍保持 22px 内边距、24px 圆角和 grid 布局，三浏览器 E2E smoke 当前为 `30 passed`，真实后端 Chromium smoke 为 `1 passed`；移动导航 1120px→390px 动态切换、Users 窄内容区、QingLong 任务数据绑定、Stock 卡片布局和 Points 导航问题已保留在历史提交中。
...
- 回归测试：Python 3.11 下 `py -3.11 -m pytest -q` 为 `123 passed`；前端 Node 测试 `npm test` 为 `36 passed`，Vitest 组件测试为 `83 passed`，Playwright mock Chromium + Firefox + WebKit smoke 共 `30 passed`，使用临时 SQLite、独立 `8001` 后端和 `VITE_API_PROXY_TARGET` 的真实后端 Chromium smoke 共 `1 passed`，`npm run lint`、扩展后的 `npm run format:check`、`npm run check:resources`、`npm run build`、`py -3.11 scripts/export_openapi.py --check` 和 compileall 均通过；npm audit、pip-audit 和许可证检查继续通过。
